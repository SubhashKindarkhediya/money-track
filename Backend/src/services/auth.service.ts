...
export class AuthService {
  ...
  async signup(data: any) {
    ...
    if (existingEmail) {
      if (!existingEmail.is_verified) {
        // Overwrite pending/unverified user details and resend OTP
        ...
        // Link Person if mobile number updated
        if (phone_number) {
          await Person.update(
            { linked_user_id: existingEmail.id },
        ...
        return { email, message: "Verification OTP resent to your email" };
      }
    ...
    if (phone_number) {
      const existingPhone = await User.findOne({ where: { phone_number, is_verified: true } });
      if (existingPhone) {
  ...
  async verifySignupOtp(data: any) {
    ...
    if (user.reset_otp_expires && new Date() > user.reset_otp_expires) {
      throw new Error("OTP has expired. Please sign up again to get a new one.");
    }
  ...
  async login(data: any) {
    ...
    if (!user.is_verified) {
      throw new Error("Please verify your email first. An OTP was sent to your email during signup.");
    }
